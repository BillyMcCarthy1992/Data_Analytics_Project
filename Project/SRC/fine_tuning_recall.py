...
best_logreg, best_logreg_params, logreg_row, logreg_cm, logreg_report = run_search(
    model_name="Logistic Regression (Tuned, Recall)",
...
all_best_params["Logistic Regression (Tuned, Recall)"] = best_logreg_params

best_tree, best_tree_params, tree_row, tree_cm, tree_report = run_search(
    model_name="Decision Tree (Tuned, Recall)",
    search=tree_search,
    X_fit=X_train,
    y_fit=y_train,
    X_eval=X_test,
...
all_best_params["Decision Tree (Tuned, Recall)"] = best_tree_params

best_rf, best_rf_params, rf_row, rf_cm, rf_report = run_search(
    model_name="Random Forest (Tuned, Recall)",
    search=rf_search,
    X_fit=X_train,
    y_fit=y_train,
    X_eval=X_test,
...
all_best_params["Random Forest (Tuned, Recall)"] = best_rf_params

best_gb, best_gb_params, gb_row, gb_cm, gb_report = run_search(
    model_name="Gradient Boosting (Tuned, Recall)",
    search=gb_search,
    X_fit=X_train,
    y_fit=y_train,
    X_eval=X_test,
...
all_best_params["Gradient Boosting (Tuned, Recall)"] = best_gb_params

# save the tuned results so I can use them later
tuned_results_df = pd.DataFrame(all_results)
tuned_results_path = TABLES_DIR / "tuned_recall_model_results.csv"
tuned_results_df.to_csv(tuned_results_path, index=False)

save_text(TABLES_DIR / "logreg_tuned_recall_classification_report.txt", logreg_report)
save_text(TABLES_DIR / "tree_tuned_recall_classification_report.txt", tree_report)
save_text(TABLES_DIR / "rf_tuned_recall_classification_report.txt", rf_report)
save_text(TABLES_DIR / "gb_tuned_recall_classification_report.txt", gb_report)

pd.DataFrame(logreg_cm).to_csv(TABLES_DIR / "logreg_tuned_recall_confusion_matrix.csv", index=False)
pd.DataFrame(tree_cm).to_csv(TABLES_DIR / "tree_tuned_recall_confusion_matrix.csv", index=False)
pd.DataFrame(rf_cm).to_csv(TABLES_DIR / "rf_tuned_recall_confusion_matrix.csv", index=False)
pd.DataFrame(gb_cm).to_csv(TABLES_DIR / "gb_tuned_recall_confusion_matrix.csv", index=False)

# save the tuned models too
joblib.dump(best_logreg, MODELS_DIR / "logreg_tuned_recall_model.joblib")
joblib.dump(best_tree, MODELS_DIR / "decision_tree_tuned_recall_model.joblib")
joblib.dump(best_rf, MODELS_DIR / "random_forest_tuned_recall_model.joblib")
joblib.dump(best_gb, MODELS_DIR / "gradient_boosting_tuned_recall_model.joblib")

with open(TABLES_DIR / "tuned_best_params_recall.json", "w", encoding="utf-8") as f:
...
if existing_df is not None:
    ...
    combined_df.to_csv(TABLES_DIR / "baseline_and_tuned_recall_results.csv", index=False)

    print(f"Loaded existing results from: {existing_path}")
    print("Saved combined baseline+tuned table to:")
    print(TABLES_DIR / "baseline_and_tuned_recall_results.csv")
    print()
...
print(tuned_results_df.sort_values(by="Recall", ascending=False))